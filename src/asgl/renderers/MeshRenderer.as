	...
	public class MeshRenderer extends BaseRenderer {
		...
		public override function pushRenderable(renderable:BaseRenderable, device:Device3D, camera:Camera3D, material:Material, staticRenderData:Vector.<AbstractStaticRenderData>):void {
			if (staticRenderData == null) {
				...
				if (ShaderPropertyType.DIFFUSE_TEX_REGION in constants) {
					var tex:AbstractTextureData = mat._textures[ShaderPropertyType.DIFFUSE_TEX];
					if (tex == null) {
						Shader3DHelper.setGlobalDiffuseTexMatrix(null, renderable._textureRegions[ShaderPropertyType.DIFFUSE_TEX]);
					} else {
						Shader3DHelper.setGlobalDiffuseTexMatrix(tex._region, renderable._textureRegions[ShaderPropertyType.DIFFUSE_TEX]);
					}
				}
