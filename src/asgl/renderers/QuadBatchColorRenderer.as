package asgl.renderers {
	...
	import asgl.shaders.scripts.builtin.priorityfill.ConstantBatchColorShaderAsset;
	import asgl.shaders.scripts.builtin.priorityfill.VertexBatchColorShaderAsset;
	...
	public class QuadBatchColorRenderer extends BaseRenderer {
		...
		private var _constantBatchMaxQuad:int;
		...
		public function QuadBatchColorRenderer(device:Device3D) {
			_device = device;
		...
		public override function pushRenderable(renderable:BaseRenderable, device:Device3D, camera:Camera3D, material:Material, staticRenderData:Vector.<AbstractStaticRenderData>):void {
			...
			if (mat != null && mat._shader != null) renderable.updateShaderProgram();
			...
			if (renderable._shaderID == 0) {
				if (_shaderID != 0) {
					...
					_numQuads = numQuads;
				} else if (_blendFactors._blendFactorsID == blendFactors._blendFactorsID && 
					_scissorRectangle == renderable.scissorRectangle) {
					isBreak = false;
				...
				if (isBreak) {
					if (_numRenderables > 0) {
						if (_numQuads > _constantBatchMaxQuad) {
		...
		public override function renderStatic(device:Device3D, renderID:uint):void {
			...
			if (sd != null) {
				...
				device._textureManager.deactiveTextures();
		...
		private function _renderCustom(device:Device3D, staticRenderData:Vector.<AbstractStaticRenderData>):void {
			if (staticRenderData == null) {
				...
				for (var i:int = 0; i < _numRenderables; i++) {
					...
					var colors:MeshElement = elements[MeshElementType.COLOR0];
					...
					for (var j:int = 0; j < len; j++) {
						...
						_vertexBatchData2Vector[index2++] = colors.values[k++];
						_vertexBatchData2Vector[index2++] = colors.values[k++];
						_vertexBatchData2Vector[index2++] = colors.values[k++];
						_vertexBatchData2Vector[index2++] = colors.values[k] * obj._multipliedAlpha;
					}
				}
		...
		private function _createStaticBatch(device:Device3D):StaticData {
			...
			var vertexBatchData1:Vector.<Number> = new Vector.<Number>(_numQuads * 16);
			...
			for (var i:int = 0; i < _numRenderables; i++) {
				...
				for (var j:int = 0; j < len; j++) {
					...
					vertexBatchData1[index2++] = colors.values[k++];
					vertexBatchData1[index2++] = colors.values[k++];
					vertexBatchData1[index2++] = colors.values[k++];
					vertexBatchData1[index2++] = colors.values[k] * obj._multipliedAlpha;
				}
			}
			...
			var vertexBatchDataBuffer1:VertexBufferData = device._vertexBufferManager.createVertexBufferData(numVertices, 4);
			vertexBatchDataBuffer1.format = Context3DVertexBufferFormat.FLOAT_4;
			...
			if (staticRenderData == null) {
				...
				for (var i:int = 0; i < _numRenderables; i++) {
					...
					for (var j:int = 0; j < len; j++) {
						...
						_constantBatchConstVector[index++] = colors.values[k++];
						_constantBatchConstVector[index++] = colors.values[k++];
						_constantBatchConstVector[index++] = colors.values[k++];
						_constantBatchConstVector[index++] = colors.values[k] * obj._multipliedAlpha;
					}
				}
		...
		private function _dilatationVertexBatch(device:Device3D, numQuads:int):void {
			...
			if (_vertexBatchIndexBuffer == null) {
				...
				_vertexBatchData2Vector = new Vector.<Number>(numVertices * 4);
			} else {
				_vertexBatchIndexBuffer.dispose();
				_vertexBatchData1Buffer.dispose();
				_vertexBatchData2Buffer.dispose();
				...
				_vertexBatchData2Vector.length = numVertices * 4;
			}
			...
			_vertexBatchData2Buffer = device._vertexBufferManager.createVertexBufferData(numVertices, 4);
			_vertexBatchData2Buffer.format = Context3DVertexBufferFormat.FLOAT_4;
		...
		private function _deviceRecoveryHandler(e:Event):void {
			...
			_vertexBatchMaterial.shader.upload(VertexBatchColorShaderAsset.asset);
			...
			_constantBatchMaterial.shader.upload(ConstantBatchColorShaderAsset.asset);
