package asgl.renderables {
	...
	public class FlashDisplayObjectRenderable extends BaseRenderable {
		...
		/**
		 * @uploadTextureHandler handler(tex:AbstractTextureData, bmd:BitmapData);
		 */
		public function upload(dis:DisplayObject, textureFormat:String=Context3DTextureFormat.BGRA, powerOfTow:Boolean=true, alignX:int=AlignType.RIGHT, alignY:int=AlignType.BOTTOM, uploadTextureHandler:Function=null):void {
			_width = dis.width;
			_height = dis.height;
			...
			if (uploadTextureHandler == null) {
				_tex.uploadFromBitmapData(bmd);
			} else {
				uploadTextureHandler(_tex, bmd);
			}
