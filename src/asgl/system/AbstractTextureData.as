package asgl.system {
	...
	public class AbstractTextureData extends DeviceData {
		...
		public function setRenderToThis(colorOutputIndex:int=-1):void {
			if (_device != null) {
				var index:int = colorOutputIndex < 0 ? this.colorOutputIndex : colorOutputIndex;
				_device.setRenderToTarget(this, enableDepthAndStencil, antiAlias, surfaceSelector, index);
			}
		}
		...
		public function uploadEmpty(miplevel:uint=0, side:uint=0):void {
			...
			} else if (_format == Context3DTextureFormat.RGBA_HALF_FLOAT) {
				bytes.length = _width * _height * 8;
