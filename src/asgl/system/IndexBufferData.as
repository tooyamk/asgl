package asgl.system {
	...
	public class IndexBufferData extends DeviceData {
		...
		asgl_protected var _bufferUsage:String;
		...
		public function IndexBufferData(device:Device3D, numIndices:int, bufferUsage:String) {
			super(device);
		...
		protected function _constructor():void {
			...
			if (context != null) {
				if (context.driverInfo == Device3D.DISPOSED) {
					...
					_buffer = context.createIndexBuffer(numIndices, _bufferUsage);
				}
			}
		}
		...
		asgl_protected override function _recovery():void {
			if (_device._context3D.driverInfo == Device3D.DISPOSED) {
				...
				_buffer = _device._context3D.createIndexBuffer(numIndices, _bufferUsage);
