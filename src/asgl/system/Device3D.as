package asgl.system {
	...
	public class Device3D extends EventDispatcher {
		...
		asgl_protected var _profile:Device3DProfile;
		...
		private var _rts:Object;
		private var _numRtt:int;
		private var _rttPool:Vector.<RT>;
		private var _numRttInPool:int;
		private var _deactiveUnoccupiedRTs:Vector.<RT>;
		private var _numDeactiveUnoccupiedRTs:int;
		...
		public function Device3D(stage:Stage3D) {
			...
			_fillMode = Context3DFillMode.SOLID;
			
			_instanceID = ++_instanceIDAccumulator;
			
			_rts = {};
			_rttPool = new Vector.<RT>();
			_deactiveUnoccupiedRTs = new Vector.<RT>();
		...
		public function get profile():Device3DProfile {
			return _profile;
		}
		...
		public function requestCreateMatchingProfiles(profiles:Vector.<String>):void {
			if (_stage != null && _isFirstRequest) {
				...
				if (context == null) {
					_stage.addEventListener(Event.CONTEXT3D_CREATE, _context3DCreatedHandler, false, 0, true);
					_stage.requestContext3DMatchingProfiles(profiles);
				} else if (context3D.driverInfo == DISPOSED) {
					_lost();
				} else {
					_isShared = true;
		...
		public function setRenderToBackBuffer():void {
			if (_numRtt > 0) {
				...
				for (; i < _numRttInPool; i++) {
					delete _rts[_rttPool[i].colorOutputIndex];
				}
		...
		public function resetOccupiedRenderTargets():void {
			for each (var rt:RT in _rts) {
				rt.occupiedValue = 0;
			}
		}
		public function deactiveUnoccupiedRenderTargets():void {
			for each (var rt:RT in _rts) {
				if (rt.occupiedValue == 0) {
					_deactiveUnoccupiedRTs[_numDeactiveUnoccupiedRTs++] = rt;
				}
			}
			
			if (_numDeactiveUnoccupiedRTs > 0) {
				for (var i:int = 0; i < _numDeactiveUnoccupiedRTs; i++) {
					setRenderToTarget(null, false, 0, 0, _deactiveUnoccupiedRTs[i].colorOutputIndex);
				}
				_numDeactiveUnoccupiedRTs = 0;
			}
		}
		public function setFillMode(fillMode:String):void {
			if (_fillMode != fillMode) {
				...
				if (_context3D != null) _context3D.setFillMode(fillMode);
			}
		}
		public function setRenderToTarget(texture:AbstractTextureData, enableDepthAndStencil:Boolean=false, antiAlias:int=0, surfaceSelector:int=0, colorOutputIndex:int=0):void {
			var tex:TextureBase;
			
			var rtt:RT = _rts[colorOutputIndex];
			if (texture == null) {
				if (rtt != null) {
					delete _rts[colorOutputIndex];
					_rttPool[_numRttInPool++] = rtt;
					_numRtt--;
				}
			} else {
				...
				if (rtt == null) {
					rtt = _numRttInPool == 0 ? new RT() : _rttPool[--_numRttInPool];
					_rts[colorOutputIndex] = rtt;
					_numRtt++;
				}
			...
			if (_context3D != null) {
				_context3D.setRenderToTexture(tex, enableDepthAndStencil, antiAlias, surfaceSelector, colorOutputIndex);
				
				_debugger._changeOtherStateCount++;
			}
		}
		public function setRenderToTargets(texs:Vector.<AbstractTextureData>):void {
			resetOccupiedRenderTargets();
			
			var len:int = texs.length;
			for (var i:int = 0; i < len; i++) {
				var tex:AbstractTextureData = texs[i];
				if (tex != null) tex.setRenderToThis();
			}
			
			deactiveUnoccupiedRenderTargets();
		}
		...
		public function testLost():void {
			if (_context3D != null) {
				if (_context3D.driverInfo == DISPOSED) _lost();
			}
		}
		...
		private function _reset(context:Context3D):void {
			...
			_context3D.setDepthTest(_depthTestMask, _depthTestPassCompareMode);
			_context3D.setBlendFactors(_blendSourceFactor, _blendDestinationFactor);
			_context3D.setFillMode(_fillMode);
			...
			if (_isFirstRequest) {
				...
				if (_numRtt > 0) {
					for each (var rtt:RT in _rts) {
						setRenderToTarget(_textureManager._textureMap[rtt.id], rtt.enableDepthAndStencil, rtt.antiAlias, rtt.surfaceSelector, rtt.colorOutputIndex);
					}
				}
...
class RT {
	...
	public var occupiedValue:int = -1;
}