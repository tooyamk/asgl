	...
	public class ShaderScriptCompiler {
		...
		public function compile(code:String, printMsg:Boolean=false):ByteArray {
			...
			for (k = 0; k < len1; k++) {
				...
				if (op.vert != null) {
					var verts:Vector.<Opcode> = op.vert[k];
					tokens += verts.length;
					vert = _programCompiler.compile(Context3DProgramType.VERTEX, verts, _memberManager, vertSettings);
				}
				
				if (op.frag != null) {
					var frags:Vector.<Opcode> = op.frag[k];
					tokens += frags.length;
					frag = _programCompiler.compile(Context3DProgramType.FRAGMENT, frags, _memberManager, fragSettings);
				}
				...
				if (vertSettings.version != fragSettings.version) {
					...
					var ver:int = AGALConfiguration.getLowestVersionWithTokens(tokens);
					if (max < ver) max = ver;
