package asgl.shaders.scripts.compiler {
	...
	public class ProgramCompiler implements IProgramCompiler {
		...
		public function ProgramCompiler() {
			...
			_regMap[BaseVariableType.FRAGMENT_DEPTH] = _config.fd;
		}
				...
				var agalOp:AGALOpcode = _opMap[opcode.func];
 				...
 				if (agalOp == null) {
					...
					if (opcode.dest == null) {
						...
						if (opcode.dest.name == BaseVariableType.FRAGMENT_DEPTH) {
		...
		private function _testVersion(info:Info):void {
			var ver:int = AGALConfiguration.getLowestVersionWithRegisterIndex(info.type, info.index);
			if (_settings.version < ver) _settings.version = ver;
		}
